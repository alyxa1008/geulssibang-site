...
ok(/btnRetryQ/.test(src) && /틀린 문제만 다시 풀기/.test(src) && /startQuiz\(QuizGen\.shuffle\(wrongIds\)\)/.test(src),
  "틀린 문제 다시 풀기 배선");
ok(!/Math\.random\(\)[^)]*\)\s*;?\s*\}\)\.sort|sort\(function\(\)\{return/.test(src),
  "페이지에 자체 섞기 없음 (전부 QuizGen 사용)");
